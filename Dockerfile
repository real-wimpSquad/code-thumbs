...
RUN dotnet tool install csharpier --tool-path ${DOTNET_TOOLS_PATH} && \
    dotnet tool install dotnet-format --tool-path ${DOTNET_TOOLS_PATH} && \
    chmod +x ${DOTNET_TOOLS_PATH}/* && \
    rm -rf /tmp/* ~/.nuget

# Python - ruff handles format+lint, removed redundant black
RUN apt-get update && apt-get install -y python3 python3-pip && \
    apt-get clean && rm -rf /var/lib/apt/lists/* && \
    pip3 install --break-system-packages ruff mypy pylint && \
...
# Rust - minimal profile (saves ~800MB vs default)
...
RUN apt-get update && apt-get install -y php php-xml php-mbstring php-curl unzip && \
    apt-get clean && rm -rf /var/lib/apt/lists/* && \
...
RUN if [ "$TARGETARCH" = "arm64" ] || [ "$TARGETARCH" = "amd64" ]; then \
      (apt-get update && apt-get install -y binutils git gnupg2 libc6-dev libcurl4-openssl-dev \
